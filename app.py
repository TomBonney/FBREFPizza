...
def load_data():
    try:
        ...
        df['FBREF name'] = df['FBREF name'].apply(lambda x: unicodedata.normalize('NFKD', x))
...
# Function to generate player links using FBREF name column
def link_generator(player_fbref_name):
    try:
        player_fbref_name = unicodedata.normalize('NFKD', player_fbref_name.strip())
        if player_fbref_name in df['FBREF name'].values:
            return df[df['FBREF name'] == player_fbref_name]['Link'].values[0]
        else:
            logging.error(f"Player {player_fbref_name} not found in the Excel file.")
            return None
    except Exception as e:
...
def get_players_data(player_fbref_name, league_option):
    try:
        ...
        link_to_player_profile = link_generator(player_fbref_name)
        ...
        if league_option == "Premier League 23/24":
            scout_report_link = f"/en/players/{player_id}/scout/12192/{player_fbref_name.replace(' ', '-')}-Scouting-Report"
        else:
            scout_report_link = f"/en/players/{player_id}/scout/365_m1/{player_fbref_name.replace(' ', '-')}-Scouting-Report"
...
def show_picture(params, values, name_of_player, player_image_url, league_option):
    try:
        ...
        fig.text(
            0.515, 0.925,
            "Percentile Rank vs Premier League Players in their Position for 2023/2024" if league_option == "Premier League 23/24" else "Percentile Rank vs Top 5 League Players in their Position",
            size=13,
            ha="center", fontproperties=font_bold.prop, color="#ffffff"
        )
...
def stats_gobbler(player_fbref_name, selected_values, league_option):
    try:
        data_scraper = get_players_data(player_fbref_name, league_option)
        ...
        show_picture(params, values, player_fbref_name, player_image_url, league_option)
    except Exception as e:
...
# Add new dropdown for league options
league_option = st.selectbox("Select an option:", ["Top 5 leagues", "Premier League 23/24"])

if league_option == "Premier League 23/24":
    league_options = ["Premier League"]
else:
    league_options = sorted(df['League'].unique().tolist())

# Create columns for the top 4 rows to span the width of the container
col1, col2 = st.columns(2)
with col1:
    league = st.selectbox("Select a league:", league_options)
...
with col3:
    players = sorted(df[df['Team'] == team]['FBREF name'].tolist())
    player_fbref_name = st.selectbox("Select a player:", players)
...
# Function to create multiple columns for each stat category
def create_stat_columns(title, options, default_values):
    cols = st.columns(5)
    selected_values = []
    for col, default_val in zip(cols, default_values):
        with col:
            selected_values.append(st.selectbox(f"{title} stat:", options, index=options.index(default_val)))
            st.markdown(
                """
                <style>
...
for title in additional_titles:
    selected_values[title] = create_stat_columns(title, dropdown_options[title], default_values[title])

if st.button("Submit"):
    all_selected_values = [val for sublist in selected_values.values() for val in sublist]
    stats_gobbler(player_fbref_name, all_selected_values, league_option)

# Add Buy Me A Coffee button as a footer
components.html(
    """
    <style>
    .footer {
    ...
    <div class="footer">
        <script type="text/javascript" src="https://cdnjs.buymeacoffee.com/1.0.0/button.prod.min.js" data-name="bmc-button" data-slug="tabonney" data-color="#FFDD00" data-emoji=""  data-font="Cookie" data-text="Buy me a coffee" data-outline-color="#000000" data-font-color="#000000" data-coffee-color="#ffffff"></script>
    </div>
    """,
    height=70,
)
